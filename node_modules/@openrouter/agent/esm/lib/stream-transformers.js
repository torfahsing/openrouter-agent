 ...
 */
function findBufferedTerminalEvent(stream) {
    return stream.findLastBuffered((event) => 'type' in event &&
 ...
 */
export function extractCompletionFromBuffer(stream) {
 ...
 * the close frame.
 */
export function tryExtractCompletionFromBuffer(stream) {
    const terminal = findBufferedTerminalEvent(stream);
    if (terminal && (isResponseCompletedEvent(terminal) || isResponseIncompleteEvent(terminal))) {
        return terminal.response;
    }
